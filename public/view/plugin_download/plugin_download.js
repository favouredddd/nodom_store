			...
			me.partical.forEach(function(i, index) {
				...
				for (var j = 0; j < tem.length; j++) {
					...
					if (i !== t && t.x && t.y) {
						...
						y = (i.y - t.y) * (i.y - t.y)
						if (x + y < t.max) {
							if (t === me.target && Math.sqrt(x + y) > Math.sqrt(i.max) / 2) {
								i.x -= (i.x - t.x) * 0.02
								i.y -= (i.y - t.y) * 0.02
							}
							me.ctx.beginPath()
...
;(function() {
	var down = DD.createModule({
			...
			first_type: [
				{
					...
					show: true,
					plugin: [
						{ name: '折线图', route: '/route/plugin_download/Chart_1', actives: true },
						{ name: '直方图', route: '/route/plugin_download/Chart_2', actives: false },
						{ name: '饼状图', route: '/route/plugin_download/Chart_3', actives: false },
						{ name: '散点图', route: '/route/plugin_download/Chart_4', actives: false },
						{ name: '雷达图', route: '/route/plugin_download/Chart_5', actives: false }
					]
				},
				{
					first_name: '缓冲动画',
					show: false,
					plugin: [
						{ name: '泡泡动画', route: '/route/plugin_download/Animation_1', actives: false },
						{ name: '方块动画', route: '/route/plugin_download/Animation_2', actives: false },
						{ name: '圆环动画', route: '/route/plugin_download/Animation_3', actives: false },
						{ name: '闪烁动画', route: '/route/plugin_download/Animation_4', actives: false },
						{ name: '水滴动画', route: '/route/plugin_download/Animation_5', actives: false },
						{ name: '线动画', route: '/route/plugin_download/Animation_6', actives: false }
					]
				},
				{
					first_name: '轮播图',
					show: false,
					plugin: [
						{ name: '常用轮播图', route: '/route/plugin_download/Carousel_1', actives: false },
						{ name: '水平旋转', route: '/route/plugin_download/Carousel_2', actives: false },
						{ name: '垂直轮播图', route: '/route/plugin_download/Carousel_3', actives: false },
						{ name: '3d轮播图', route: '/route/plugin_download/Carousel_4', actives: false }
					]
				},
				{
					first_name: '分页',
					show: false,
					plugin: [
						{ name: '常见', route: '/route/plugin_download/Page_1', actives: false },
						{ name: '酷炫', route: '/route/plugin_download/Page_2', actives: false }
					]
				},
				{
					first_name: '选择框',
					show: false,
					plugin: [
						{ name: '常见', route: '/route/plugin_download/Checkbox_1', actives: false },
						{ name: '方形', route: '/route/plugin_download/Checkbox_3', actives: false },
						{ name: '眼睛', route: '/route/plugin_download/Checkbox_4', actives: false },
						{ name: '心形', route: '/route/plugin_download/Checkbox_5', actives: false }
					]
				},
				{
					first_name: '日期选择',
					show: false,
					plugin: [
						{ name: '日历', route: '/route/plugin_download/Date_1', actives: false },
						{ name: '年月日', route: '/route/plugin_download/Date_2', actives: false }
					]
				},
				{
					first_name: '折叠',
					show: false,
					plugin: [{ name: '普通折叠', route: '/route/plugin_download/foldCollapse_1', actives: false }]
				},
				{
					first_name: '图片放大镜',
					show: false,
					plugin: [{ name: '高清版本', route: '/route/plugin_download/Magn_1', actives: false }]
				},
				{
					first_name: '自动补全',
					show: false,
					plugin: [{ name: '多用补全', route: '/route/plugin_download/Complete_1', actives: false }]
				},
				{
					first_name: '进度条',
					show: false,
					plugin: [
						{ name: '普通', route: '/route/plugin_download/Progress_2', actives: false },
						{ name: '横向拖动', route: '/route/plugin_download/Progress_1', actives: false },
						{ name: '圆环', route: '/route/plugin_download/Progress_3', actives: false }
					]
				},
				{
					first_name: '开关',
					show: false,
					plugin: [
						{ name: '3D按钮开关', route: '/route/plugin_download/Switch_3', actives: false },
						{ name: '3D开关', route: '/route/plugin_download/Switch_2', actives: false },
						{ name: '普通开关', route: '/route/plugin_download/Switch_1', actives: false }
					]
				},
				{
					first_name: '地址',
					show: false,
					plugin: [{ name: '多用地址', route: '/route/plugin_download/Location_1', actives: false }]
				},
				{
					first_name: '表格',
					show: false,
					plugin: [{ name: '多用表格', route: '/route/plugin_download/Table_1', actives: false }]
				},
				{
					first_name: '菜单树',
					show: false,
					plugin: [{ name: '树组件', route: '/route/plugin_download/Tree_1', actives: false }]
				},
				{
					first_name: '颜色选择器',
					show: false,
					plugin: [{ name: '选择器', route: '/route/plugin_download/ColorPicker_1', actives: false }]
				},
				{
					first_name: '按钮',
					show: false,
					plugin: [{ name: '按钮列表', route: '/route/plugin_download/Button_1', actives: false }]
				},
				{
					first_name: '图标库',
					show: false,
					plugin: [{ name: '图标列表', route: '/route/plugin_download/Icon_1', actives: false }]
				}
			]
		},
		onBeforeFirstRender: function() {
			...
			me.data.first_type[0].plugin[0].actives = true
			me.data.top = false
		},
		onFirstRender() {
			var me = this
			...
			if (window.pageYOffset) {
				if (document.querySelector('.route')) {
					if (window.pageYOffset > 80) {
						down.model.data.top = true
					} else {
						down.model.data.top = false
					}
				}
			}
		},
