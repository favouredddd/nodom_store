...
var DD = {
    ...
    getAttrs: function(el, reg) {
        ...
        for (var i = 0; i < el.attributes.length; i++) {
            ...
            if (reg.test(attr.name)&&attr.value) {
                arr.push(attr);
            }
        }
        return arr;
    ...
    request: function(config) {
        ...
        if (window.MX && config.url.indexOf("http://") !== 0 && config.url.indexOf("https://") !== 0) {
            MX.invoke("MXFileSystem", "readAssetFile", {
                fileName: config.url
            }, function(r) {
                r = r.result;
                if (config.type === 'json') {
...
DD.config = {
    ...
    appPath: '', // 应用加载默认路径,
    deviceType: IsPC() ? 2 : 1 // 设备类型 1:触屏，2:pc
};
...
(function() {
    ...
    function initrepeat(value) {
        ...
        // 增加x-model指令
        DD.Directive.initViewDirective(tnode, [{
            name: 'model',
            value: modelName
        }]);
...
(function() {
    ...
    DD.assign(Module, {
        ...
        define: function(config) {
            ...
            // 存储类
            me.classFactory[cname] = DD.merge({
                virtualDom: null
            }, config);
    ...
    DD.Router = {
        ...
        clearHistory: function(saveLength) {
            ...
            setTimeout(function() {
                ...
                // 设置新的state
                me.currentState = {
                    path: me.histories[me.history - 1],
                    index: me.history,
                    forward: true
                };
            }, 20);
...
(function() {
    ...
    /**
     * @param config
     *            el: element selector module: 模块 model: 模型
     */
