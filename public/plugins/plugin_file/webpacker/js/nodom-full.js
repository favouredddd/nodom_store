...
(function () {
    ...
    window.addEventListener('popstate', function (e) {
        ...
        // if(DD.Router.jump)
        //      return ;
        // DD.Router.jump=false;
        // console.log(state,me.histories);
        if (!state) {
            return;
        }
...
(function(){
    ...
    Chart.prototype.drawScatter = function () {
        ...
        for(var i = 0, l = me.data.length; i < l; i++) {
            var datai = me.data[i].datas;
            var color = getColor(me, i);
            for(var j = 0, jl = datai.length; j < jl; j++) {
                var circle = DD.newSvgEl('circle');
                graphics.appendChild(circle);
                var x = (datai[j].x-minx)*px1;
                var y = height - (datai[j].y-miny)*py1;
    ...
    Chart.prototype.drawRadar = function () {
        ...
        if (me.radarName) {
            me.radarData = model.data[me.radarName];
        }
        ...
        // 顶点个数
        var TITLE_LEN = me.radarData.titles.length;
        // 数据最大值并向上取10整
        var MAX_VALUE = getMaxValue(me.data);
        // 顶点字符串最大宽度
        ...
        drawAxes(me.radarData.titles, CX, CY, RADIUS);
        drawData(me.data);

        function drawAxes(titles, rx, ry, raduis) {
            var i;
            var len = titles.length;
            var points = [];
            var lineColor = me.radarData.lineColor;
            for(i=0; i<len; i++) {
                var angle = Math.PI * 2 * i / len - Math.PI/2; // 90度修正
                var cos = Math.cos(angle);
                var sin = Math.sin(angle);
                var cx = cos*raduis+rx;
            ...
            for(i=5; i>=1; i--) {
                ...
                var pointStrs = [];
                for(var j=0, l=trigonometrics.length; j<l; j++) {
                    var scale = i / 5;
                    var point = trigonometrics[j];
                    var px = point[0]*raduis*scale + rx;
                    var py = point[1]*raduis*scale + ry;
                    pointStrs.push(px + ',' + py);
                }
                DD.attr(p, {
                    points: pointStrs.join(' '),
                    fill: getRadarColor(me.radarData.colors, i),
                    stroke: lineColor,
                    'stroke-width': 1
                })
            }
            for(i=0; i<len; i++) {
                var line = DD.newSvgEl('line');
                var text = DD.newSvgEl('text');
                var po = points[i];
                var textHtml = titles[i];
                var textLen = textHtml.length * WORDLEN;
        ...
        function drawData(datas) {
            for(var i=0, l=datas.length; i<l ;i++) {
                var data = datas[i].datas;
                var pointStrs = [];
                var color = getColor(me, i);
                var polygon = DD.newSvgEl('polygon');
                graphics.appendChild(polygon);
                for(var j=0; j < TITLE_LEN; j++) {
                    var v = data[j] || 0;
                    var scale = v / MAX_VALUE;
                    var trigonometric = trigonometrics[j];
                    var px = trigonometric[0]*RADIUS*scale + CX;
                    var py = trigonometric[1]*RADIUS*scale + CY;
                    pointStrs.push(px+','+py)
                }
                DD.attr(polygon, {
                    points: pointStrs.join(' '),
        ...
        function getMaxValue(datas) {
            var maxs = [];
            for(var i = 0, l = datas.length; i < l; i++) {
                maxs.push(Math.max.apply(null, datas[i].datas));
            }
            var maxv =  Math.max.apply(null, maxs);
            return Math.ceil(maxv / 10) * 10
        }
        // 获取文字的最大长度
        function getTitleMaxLen(titles) {
            var max = 0;
        ...
        // 获取颜色
        function getRadarColor(colors, index) {
